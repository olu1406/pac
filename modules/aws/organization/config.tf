...
# Organization-wide configuration compliance monitoring

# S3 bucket for Config
resource "aws_s3_bucket" "config" {
  bucket        = "${var.organization_name}-config-${random_id.config_bucket_suffix.hex}"
  force_destroy = false
...
# Generate random suffix for Config bucket name
...
# KMS key alias for Config
resource "aws_kms_alias" "config" {
  name          = "alias/${var.organization_name}-config"
...
# Additional policy for Config to access S3 and KMS
resource "aws_iam_role_policy" "config_s3_kms" {
  name = "ConfigS3KMSPolicy"
